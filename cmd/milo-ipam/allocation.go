...
` + scopeGrammarHelp(),
  ...
  datumctl ipam allocation list --unclaimed`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if purpose != "" && purpose != string(ipamv1alpha1.PurposeClaim) && purpose != string(ipamv1alpha1.PurposeReservation) {
				return usageErrorf("invalid --purpose %q: must be %s or %s",
					purpose, ipamv1alpha1.PurposeClaim, ipamv1alpha1.PurposeReservation)
			}
			...
			for i := range list.Items {
				...
				if purpose != "" && string(al.Spec.Purpose) != purpose {
					continue
				}
	...
	f.StringVar(&purpose, "purpose", "", "Only show allocations of this purpose: Claim|Reservation")
...
func allocationClaimName(al *ipamv1alpha1.IPAllocation) string {
	...
	if al.Spec.Purpose == ipamv1alpha1.PurposeReservation {
		return "— (reserved)"
	}
	return "— (retained)"
}
...
// as `address show` and concealed a doubly-held address in exactly the same way.
// A name resolves to at most one object by construction.
...
func (a *app) renderAllocationDetail(al *ipamv1alpha1.IPAllocation) error {
	...
	if al.Spec.OwnerRef != nil {
		t.row("Held for", formatObjectRefWithUID(al.Spec.OwnerRef))
	}
...
func newAddressShowCommand(a *app) *cobra.Command {
  ...
  datumctl ipam address show 2001:db8:1::/96`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			} else if done, mErr := a.renderMachine(al, func() string { return "ipallocation/" + al.Name }); done {
				return mErr
			}
...
//
// It used to return on the first exact match, which made it the one tool that
// could not report the one condition it would be reached for. Two root pools
// over one range hand the same address to unrelated claims (#87), and asked who
// held such an address this command named a single claim and gave no hint a
// second existed. Both allocations are /32, so the tightest-match tiebreak below
// picked between them arbitrarily — the answer was not merely incomplete, it was
// unstable.
//
// A diagnostic that hides the condition it would be used to diagnose is worse
// than no diagnostic, because it converts an investigation into a wrong
// conclusion. Collecting every match costs one extra comparison over a list this
// already walks in full.
func (a *app) lookupAllocationsByAddress(cs clientset.Interface, ns, arg string) ([]*ipamv1alpha1.IPAllocation, error) {
	...
	for i := range list.Items {
		...
		// one address is the same collision as two exact matches, and keeping only
		// the first is what made the old answer arbitrary.
		switch {
		case bits > bestBits:
	...
	// Exact matches beat an enclosing block, as before. The difference is that
	// all of them are returned rather than whichever the list happened to yield
	// first.
	if len(exact) > 0 {
		return exact, nil
	}
	...
	return nil, newCLIError(exitNotFound,
		...
		withFix(a.addressNotFoundFix(arg, ns))
}
...
// can be configured with it and put on one wire, and the only way it currently
// arises is two root pools over one range — which nothing else surfaces: no
// condition, no event, no metric, and `allocation list` shows it only to someone
// who already suspects and sorts by address.
//
...
func (a *app) warnMultipleHolders(arg string, holders []*ipamv1alpha1.IPAllocation) {
	...
	for _, h := range holders {
		...
		fmt.Fprintf(a.io.ErrOut, "         %-24s pool %-20s class %-20s %s\n",
...
//
// Note there is deliberately no "widen the search" suggestion: no cluster-wide
// reverse lookup exists, and pointing at `allocation list` implies one does.
func (a *app) addressNotFoundFix(arg, ns string) string {
	var b strings.Builder
	b.WriteString("the reverse lookup reads one namespace. If the address belongs to a\n")
	fmt.Fprintf(&b, "       different one, name it:\n")
...
func (a *app) renderAddressAnswer(arg string, al *ipamv1alpha1.IPAllocation, cs clientset.Interface) error {
	...
	case al.Spec.OwnerRef != nil:
		_, _ = fmt.Fprintf(a.io.Out, "  claimed by: %s\n", formatObjectRefWithUID(al.Spec.OwnerRef))
		if al.Spec.ClaimRef != nil {
