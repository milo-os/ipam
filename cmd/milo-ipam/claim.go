...
func newClaimCreateCommand(a *app) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use: "create",
		// `prefix claim` was the old spelling of this command.
		Aliases: []string{"claim", "request"},
...
` + scopeGrammarHelp(),
  ...
  datumctl ipam claim create --class tenant-endpoint-ipv4 \
	...
	f.StringVar(&o.owner, "owner", "", "Consumer object this address is held for, as Kind.apiGroup/name[#uid]")
...
func (a *app) preflightClaimScope(cs clientset.Interface, claim *ipamv1alpha1.IPClaim) error {
	...
	// everything". A class legitimately requires no scope, and until the server
	// computes the field the wire value is empty for every class — so the loop
	// below must stay a check of what the class asked for, not an assertion that
...
func buildClaim(o *claimOptions, ns string) (*ipamv1alpha1.IPClaim, error) {
	...
	claim := &ipamv1alpha1.IPClaim{
		...
		Spec: ipamv1alpha1.IPClaimSpec{
			...
			Scope:         scope,
			OwnerRef:      owner,
			ReclaimPolicy: ipamv1alpha1.ReclaimPolicy(o.reclaimPolicy),
		},
	}
	if o.prefixLength > 0 {
...
` + scopeGrammarHelp(),
  ...
  datumctl ipam claim list --scope network=default`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Filtering by pool is a read-side question ("what is drawing on this
			// pool"), not a way to pick one — status.poolRef is the server's answer.
...
// scopeContains reports whether have includes every role/name pair in want. UID
// is compared only when the filter supplies one, so `--scope network=default`
// matches a claim whose reference is UID-pinned.
...
func resolveClaim(cs clientset.Interface, ns, arg string) (*ipamv1alpha1.IPClaim, error) {
	if looksLikeAddress(arg) {
		...
		// The reverse lookup is offered for exactly one of the two reasons this
		// search comes up empty: an address held with no claim behind it (a Retain
		// release) is invisible to a claim search and is precisely what `address
		// show` answers, in this same namespace.
		//
		// It used to be offered for the other reason as well — "allocated in
		// another project" — and that was circular. `address show` reads the same
		// tenant keyspace this List just read, so it cannot see another project's
		// address either; following the advice reached a second not-found that
		// pointed back here. There is no cross-project search to hand off to (see
		// addressNotFoundFix in allocation.go), so the limit is stated rather than
		// deferred to a command that shares it.
...
func (a *app) renderClaimDetail(c *ipamv1alpha1.IPClaim) error {
	...
	if c.Spec.OwnerRef != nil {
		t.row("Held for", formatObjectRefWithUID(c.Spec.OwnerRef))
	}
...
owner, until something releases it explicitly.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			retained := claim.Spec.ReclaimPolicy == ipamv1alpha1.ReclaimRetain
			fate := "frees"
			if retained {
			...
			if retained {
				_, _ = fmt.Fprintf(a.io.Out, "  %s is retained and still held; release it with `ipam allocation release`.\n", addr)
			}
			return nil
		},
	}
