...
func (a *app) renderClassTable(classes []ipamv1alpha1.IPClass) error {
	...
	if wide {
		headers = []string{"NAME", "FAMILY", "PARENT", "UNIT", "POOLS", "PROVISIONED", "REQUIRED SCOPE", "UNIQUE WITHIN", "VISIBILITY", "PHASE", "AGE"}
	}
	...
	for i := range classes {
		...
		if wide {
			t.row(name, orDash(string(c.Spec.IPFamily)), orDash(c.Spec.ParentClassName),
				...
				orDash(c.Spec.Visibility), phaseText(string(c.Status.Phase)),
...
func (a *app) renderClassDetail(c *ipamv1alpha1.IPClass, cs clientset.Interface) error {
	...
	if c.Spec.Visibility != "" {
		t.row("Visibility", c.Spec.Visibility)
	}
