...
// ----------------------------------------------------------------------------
// ASN — table-driven coverage of all branches
// ----------------------------------------------------------------------------

func TestASNPool_Allocate_Table(t *testing.T) {
	cases := []struct {
		name     string
		ranges   []ASNRange
		existing []int64
		...
		{
			name:   "empty_pool_returns_start",
			ranges: []ASNRange{{Start: 64512, End: 64520}},
			want:   64512,
		},
		{
			name:     "partially_filled_advances_past_existing",
			ranges:   []ASNRange{{Start: 100, End: 110}},
			existing: []int64{100, 101, 102},
			want:     103,
		},
		{
			name:     "fills_first_gap",
			ranges:   []ASNRange{{Start: 100, End: 110}},
			existing: []int64{100, 101, 103},
			want:     102,
		},
		{
			name:     "fully_allocated_single_range",
			ranges:   []ASNRange{{Start: 100, End: 102}},
			existing: []int64{100, 101, 102},
			wantErr:  ErrPoolExhausted,
		},
		{
			name:     "single_asn_range_empty",
			ranges:   []ASNRange{{Start: 65000, End: 65000}},
			want:     65000,
		},
		{
			name:     "single_asn_range_full",
			ranges:   []ASNRange{{Start: 65000, End: 65000}},
			existing: []int64{65000},
			wantErr:  ErrPoolExhausted,
		},
		{
			name: "rolls_to_next_range_when_first_full",
			ranges: []ASNRange{
				{Start: 100, End: 102},
				{Start: 200, End: 202},
			},
		...
		{
			name: "non_contiguous_ranges_skip_gap",
			ranges: []ASNRange{
				{Start: 100, End: 100},
				{Start: 65000, End: 65010},
			},
			existing: []int64{100},
			want:     65000,
		},
		{
			name: "all_ranges_full",
			ranges: []ASNRange{
				{Start: 1, End: 2},
				{Start: 10, End: 11},
			},
			existing: []int64{1, 2, 10, 11},
			wantErr:  ErrPoolExhausted,
		},
		{
			name:    "invalid_range_end_before_start",
			ranges:  []ASNRange{{Start: 200, End: 100}},
			wantErr: ErrInvalidASNRange,
		},
		{
			name: "existing_outside_range_is_ignored",
			ranges: []ASNRange{
				{Start: 100, End: 102},
			},
			// 50 < range.Start so it's filtered by the binary search; allocator
			// still picks 100.
			existing: []int64{50, 99},
			want:     100,
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			p := &ASNPool{Ranges: c.ranges, Existing: c.existing}
			got, err := p.Allocate()
			if c.wantErr != nil {
				if !errors.Is(err, c.wantErr) {
					t.Fatalf("err = %v, want %v", err, c.wantErr)
...
func TestASNPool_Release_Table(t *testing.T) {
	cases := []struct {
		name     string
		existing []int64
		release  int64
		...
		{
			name:     "release_missing_returns_error",
			existing: []int64{100, 102},
			release:  101,
			wantErr:  ErrNotInPool,
		},
		{
			name:     "release_from_empty_returns_error",
			existing: nil,
			release:  100,
			wantErr:  ErrNotInPool,
		},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			p := &ASNPool{
				Ranges:   []ASNRange{{Start: 100, End: 200}},
				Existing: c.existing,
			}
			got, err := p.Release(c.release)
			if c.wantErr != nil {
				if !errors.Is(err, c.wantErr) {
					t.Fatalf("err = %v, want %v", err, c.wantErr)
				}
			...
			if !int64SlicesEqual(got, c.want) {
...
func TestASNPool_Available_Table(t *testing.T) {
	cases := []struct {
		name     string
		ranges   []ASNRange
		existing []int64
		...
		{
			name:   "empty_pool_full_capacity",
			ranges: []ASNRange{{Start: 100, End: 109}},
			want:   10,
		},
		{
			name:     "two_consumed",
			ranges:   []ASNRange{{Start: 100, End: 109}},
			existing: []int64{100, 105},
			want:     8,
		},
		{
			name: "multi_range_sum_minus_existing",
			ranges: []ASNRange{
				{Start: 100, End: 102}, // 3
				{Start: 200, End: 204}, // 5
			},
			existing: []int64{100, 200, 201},
			want:     5, // 8 - 3
		},
		{
			name:     "existing_outside_ranges_not_counted",
			ranges:   []ASNRange{{Start: 100, End: 102}},
			existing: []int64{50, 100, 999},
			want:     2,
		},
		{
			name:    "invalid_range_skipped",
			ranges:  []ASNRange{{Start: 200, End: 100}, {Start: 1, End: 2}},
			want:    2,
		},
		{
			name:     "fully_consumed_returns_zero",
			ranges:   []ASNRange{{Start: 1, End: 1}},
			existing: []int64{1},
			want:     0,
		},
	}
