...
func TestASNPool_Allocate_Empty(t *testing.T) {
	p := &ASNPool{Ranges: []ASNRange{{Start: 100, End: 110}}}
	got, err := p.Allocate()
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
...
func TestASNPool_Allocate_FillsHole(t *testing.T) {
	p := &ASNPool{
	...
	if got != 102 {
		t.Fatalf("expected 102 (hole), got %d", got)
	}
}

func TestASNPool_Allocate_MultiRange(t *testing.T) {
	p := &ASNPool{
		Ranges: []ASNRange{
			{Start: 100, End: 102},
			{Start: 200, End: 202},
	...
	if got != 200 {
		t.Fatalf("expected 200 (rolls to next range), got %d", got)
	}
}

func TestASNPool_Allocate_Exhausted(t *testing.T) {
	p := &ASNPool{
...
func TestASNPool_Release_Reallocate(t *testing.T) {
	p := &ASNPool{
		Ranges:   []ASNRange{{Start: 100, End: 102}},
		Existing: []int64{100, 101, 102},
	}
	released, err := p.Release(101)
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if len(released) != 2 || released[0] != 100 || released[1] != 102 {
		t.Fatalf("expected [100,102], got %v", released)
	}
	p.Existing = released
	got, err := p.Allocate()
	if err != nil {
	...
	if got != 101 {
		t.Fatalf("expected reallocation of 101, got %d", got)
	}
}

func TestASNPool_Release_NotInPool(t *testing.T) {
	p := &ASNPool{
		Ranges:   []ASNRange{{Start: 100, End: 110}},
...
func TestASNPool_Available(t *testing.T) {
	p := &ASNPool{
		Ranges:   []ASNRange{{Start: 100, End: 109}}, // 10 ASNs
		Existing: []int64{100, 105},
	}
	if got := p.Available(); got != 8 {
		t.Fatalf("expected 8 available, got %d", got)
	}
}
